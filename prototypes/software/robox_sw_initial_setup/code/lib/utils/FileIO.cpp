...
namespace Utils
{
    /* static */ bool FileIO::readFile(Path path, Buffer buffer, bool fromRoot, int16_t index)
    {
        ...
        try
        {
            if (!exists(filePath))
            {
                Utils::Console::error("file does not exit", path);
                return false;
            }
            else
            {
        ...
        {
            std::vector<std::string> error = {e.what(), path};
            Utils::Console::error("file system error", error, true);
            return false;
        }
        return true;
    }
    /* static */ bool FileIO::createFile(Path path, Name name, bool fromRoot)
    {
        ...
        try
        {
            if (exists(filePath))
            {
                Utils::Console::error("file already exist!", path);
                return false;
            }
            else
            {
    ...
    /* static */ bool FileIO::removeFile(Path path, bool fromRoot)
    {
        ...
        try
        {
            if (!exists(filePath))
            {
                Utils::Console::error("file does not exist!", path);
                return false;
            }
            else
            {
    ...
    /* static */ bool FileIO::readDirectory(Path path, Buffer buffer, bool fromRoot)
    {
        ...
        try
        {
            if (!exists(dirPath))
            {
                Utils::Console::error("directory does not exist!", path);
                return false;
            }
            else
            {
    ...
    /* static */ bool FileIO::createDirectory(Path path, Name name, bool fromRoot)
    {
        ...
        try
        {
            if (exists(dirPath))
            {
                Utils::Console::error("directory already exist!", path);
                return false;
            }
            else
            {
